...
class PaginationViewController<T> {
  ...
  PaginationViewController({
    this.paginationSize = ConfigurationData.pageSize,
  });
...
class _PaginationViewComponentState<T> extends State<PaginationViewComponent<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.enableRefreshPull) {
      return RefreshIndicator(
        onRefresh: () async {
          if (mounted) await widget.onRefresh?.call(context);
          if (mounted) await widget.controller.reset(context);
        },
  ...
  Widget get _listStyleComponent {
    final additionalSize = widget.header == null ? 0 : 1;

    return ListView.builder(
      physics: const AlwaysScrollableScrollPhysics(),
      controller: _scrollController,
      itemCount: widget.controller.dataList.length + 1 + additionalSize,
      itemBuilder: (context, position) {
        if (position < additionalSize) {
          return widget.header ?? const SizedBox();
        } else if (widget.controller.dataList.length > position - additionalSize) {
          return widget.itemBuilder(
            context,
            PaginationViewModel(
              model: widget.controller.dataList[position - additionalSize],
              position: position - additionalSize,
            ),
          );
